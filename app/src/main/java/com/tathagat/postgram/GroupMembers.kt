...
class GroupMembers : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        query=groupreference
        mRecyclerView=findViewById(R.id.membersRecyclerView)
        mToolbar=findViewById(R.id.membersToolbar)
...
showGroupMembers()
    ...
    private fun showGroupMembers() {
        val option = FirebaseRecyclerOptions.Builder<myFriends>()
            .setQuery(groupreference!!, myFriends::class.java)
            .build()
        val firebaseRecyclerAdapter = object : FirebaseRecyclerAdapter<myFriends, MyViewHolder>(option) {
            ...
            override fun onBindViewHolder(holder: MyViewHolder, position: Int, model: myFriends) {
                var memberid=getRef(position).key.toString()
                userreference?.child(memberid)?.addValueEventListener(object : ValueEventListener {
                    ...
                    override fun onDataChange(p0: DataSnapshot?) {
                        ...
                        holder.itemView.setOnClickListener {
                            ...
                            intent.putExtra("reciever_id", memberid)
